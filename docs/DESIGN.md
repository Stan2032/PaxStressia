...
**Implemented v0.22.** Seven `any_junta`-gated event cards now ship this layer as playable content, in both the base deck (→ base + grand) and the Sahel-arc deck (the primary playable): **The Cordon** (Moura, the atrocity as aftermath-and-number), **What Everyone Knows** (the FIMI denial that follows it — §20.3), **Until Further Notice** (party dissolution by decree), **Mobilised** (punitive conscription of critics), **The Quiet** (the media-ban ladder — *quiet is not calm*), **Who Is Left to Count** (the UN panel vetoed, oversight ended — §20.4), and **Struck Off** (the sobering 2026 sanctions reversal: documentation exposes but rarely punishes; the durable win is the record). Each is rendered soberly (no gore, §2), grounded in the documented record (sourced and confidence-flagged in `rules/sources.json`, §20.6), and built as a **choice between bearing witness — funding documentation into the §20.4 Exposure track — and looking away** (which costs Local legitimacy and feeds grievance). Calibration-safe: the cards fire only once juntas exist and their default-choice effects never move the §12 junta-timing baseline — **history calibration still 10/10**. This is the moral weight Stan asked for at the outset, made mechanical: the realities of authoritarian rule are not narrated *at* the player; they arrive as decisions the player must make with their instruments.
...
- **v0.22** — 2026-06-17 — **The Closed Society layer ships as content (§20.2): the moral weight, made mechanical.** Per Stan's founding ask — *remind the player of the harsh, brutal realities citizens of authoritarian states endure* — seven `any_junta`-gated event cards now surface the documented record in both the base deck (base + grand) and the Sahel-arc deck: **The Cordon** (the Moura massacre, ≥500 killed, as aftermath-and-number), **What Everyone Knows** (the African-Initiative FIMI denial that follows it), **Until Further Notice** (party dissolution by decree), **Mobilised** (punitive conscription of critics), **The Quiet** (the media-ban ladder — *quiet is not calm*), **Who Is Left to Count** (the UN Panel vetoed, oversight ended), **Struck Off** (the 2026 sanctions reversal — documentation exposes but rarely punishes). Each is sober (no gore, §2), grounded and confidence-flagged in `rules/sources.json` (§20.6), and a **choice between bearing witness — funding the §20.4 Exposure track — and looking away** (which costs Local legitimacy and feeds grievance). Built from existing ops (added `exposure` to the event-effect schema enum). Grounded in a verified research pass (UN OHCHR; HRW; CPJ/RSF; EEAS–VIGINUM). Calibration-safe: the cards fire only after juntas form and don't move the junta-timing baseline — **calibration still 10/10; 103 tests pass / 0 xfail.** Honest note on a discarded path: a coalition inner/outer-ring tiering was prototyped and **reverted** — it measured ~0 effect because Regional Commands don't persist long enough for the nuance to matter (the real lever is grand-mode command persistence, a future pass).
- **v0.21** — 2026-06-17 — **The panels speak in objects too (UI pass 5).** The last text-heavy corner — the action list and region detail — gets the same object/icon treatment as the header and map, so the whole UI is consistent. Costs read **🏛️ n · 💰 n** (not "n Mandate / n Funds"); each action's family is an icon (⚔️ force · 🏗️ build · 🕵️ intel · 🤝 talk · 🏛️ home), its backfire a **⚠️ %** with the channel moved to the tooltip, and its target a 📍; the effect-preview tags lead with the same icons (🏠 Home · 🤝 Allies · 🛡️ Local · 🔥 Grievance · ⛓️ Drift…); the region card's stats are iconified (💥 Forces · 🏚️ Dug-in · ⚡ Activity · 🛡️ Local support · 🔦 Exposure · 🏴 Foreign patron…). Doctrinal terms stay in the tooltips (the §13.3 gloss rule). Proto/UI only — no engine/rules/calibration change. This completes the "interactive & intuitive" arc (objects → situation-map → pan/zoom → motion → consistent object-panels).
- **v0.20** — 2026-06-17 — **Motion — the map reacts to a turn (UI pass 4).** Change is now *shown*, not buried in the ledger: the board snapshots itself before a turn resolves, and after it re-renders, the nodes that actually changed get a one-shot animation — a **capital falling flashes** white, a theatre where **the insurgency flared pulses** its heat ring, a **newly-planted command unfurls** its flag. Implemented by diffing the pre/post snapshot (government rank worsened, estimated strength jumped >8, a new theatre command) and adding transient CSS-keyframe classes to exactly those node groups, auto-removed before any later re-render. Honours `prefers-reduced-motion`. Pure proto interaction — no engine/rules/calibration change. This is the legibility kind of motion (it makes consequences readable); decorative/production motion is the Godot client's.
- **v0.19** — 2026-06-16 — **Pan & zoom — the 40-nation board becomes legible up close (UI pass 3).** The dense grand map's one real gap (overlap) is solved the intuitive way: the SVG `viewBox` is now drag-to-pan, pinch / wheel / button to zoom (pointer events, with `getScreenCTM` for cursor-anchored zoom and tap-vs-drag so a pan never mis-selects). Zoomed out, the board speaks in colour + heat and you tap a nation to identify it; **zoom in and the names return** (few are on screen, so there's room — a re-render across the `view.w < 46` threshold). A `+ / − / ⤢` control sits on the map for discoverability. Pure proto interaction — no engine/rules/calibration change; `preview.mjs` gained a `viewBox` arg to render a zoomed theatre for review. This closes the §13.3 "interactive and intuitive" pass; production pan/zoom + motion remain the Godot client's to inherit.
