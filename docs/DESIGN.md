...
- **Spread (v0.4):** where `strength > `**spread_threshold**, each open edge carries `flow = `**k_spread**` × strength × capacity × (1 − interdiction) × (grievance_neighbor/100)` into the neighbor — seeding presence where none existed. Spread is *replication, not movement*: momentum recruits locally and the source keeps its strength. The arc's historical test: northern Burkina must ignite from Mopti around the 2015–16 window without being scripted.
