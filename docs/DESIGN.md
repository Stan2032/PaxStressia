...
~20 nations to start (expanding toward the §4 target of ~40–50), each a capital node grouped by `theater` (sahel, maghreb, horn, gulf, levant, afpak, caucasus, central_asia, se_asia, andes, east/west Africa). Quarter turns, a 2001 start, a 25-year (100-turn) horizon. Composited *global* faction families — an al-Qaeda-network composite, an IS-lineage composite, an ethno-separatist composite, a narco-insurgency composite — seeded across their real theaters. **Inter-theater edges** (ideology, diaspora, sea lane, arms route) make the world a single connected graph, not isolated regions: contagion, sponsor flow, and bloc adjacency cross theaters. *Honest scope: grand mode is the systems at world scale; it is **not** history-calibrated like Scenario 1 (a future milestone). Node count, markets, and multi-patron rivalry deepen in follow-ups.*
...
### 21.3 What's next for grand mode (honest backlog)
Node count toward ~45; **arms & commodity markets** (a global price layer that actions move and that feeds sponsor flow/treasury); **multiple simultaneous patrons** with a global rivalry score (your loss in one theater is a patron's gain everywhere); cross-theater **tactic/ideology contagion** (capability tier-ups propagating along ideology edges); per-theater reporting and a pan/zoom map UI; and eventually a grand-mode history-calibration pass. This section is the spec those build against.
