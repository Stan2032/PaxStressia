...
- **v0.24** — 2026-06-26 — **Balance audit — the horizon-dominance finding, surfaced not hidden** (DESIGN §19.7). New player-side instrument **`harness/audit.py`** (the companion to `calibrate.py`: calibrate grades the passive *world* against history, audit grades the *player* — score spread, ending mix, the §19.7 no-dominant-strategy gate) re-runs every archetype at **both** the 120-turn subset the enforced thesis test checks and the arc's full **168-turn horizon**. Finding: **§19.7 holds @120 but breaks @168** — pure hearts-minds (≈8.3) overtakes balanced play (≈1.3) because sustained security's per-turn spend + casualty cost is charged every turn while the stabilization it buys is bounded → "hold the capitals by cutting grievance, cede the countryside" out-economises balanced containment. Named, not papered over: a 5th thesis assertion `test_no_pure_strategy_dominates_at_the_full_horizon` asserts the destination and **xfails** until closed (the xfail test is the gate; the audit harness is report-only — never fails CI). The fix is the next milestone: a *quiet ≠ peace at country scale* scoring pass (conceded territory scores as **unstabilized**, not merely dropped from the average), to be done so the 10/10 calibration + the 120-turn gate stay true. Docs + tests + harness only; no rules/sim/proto/calibration change → calibration 10/10; **104 tests / 1 xfail** (the named destination).
