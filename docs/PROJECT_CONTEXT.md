...
- **v0.9.2** — 2026-06-14 — Map label overflow fixed (SVG text sizing + left-flip + truncation), from Stan's screenshot. Proto-only.
- **v0.10** — 2026-06-15 — **Grand mode foundation** (Stan: "go fully global now"). `rules/scenarios/grand/` (~20 nations, ~11 theaters, global composited factions, inter-theater edges); **`sim/norms.py`** global precedent layer (kinetic/law/autocracy norms → recruitment in every theater + International + patron appeal), gated by `norm_feedback` (0 in single-theater → Sahel still 10/10; >0 in grand). Both engines; `tests/test_grand.py` (8). Scope honest: world-scale systems, not yet history-calibrated; markets/multi-patron/~45 nodes backlog (DESIGN §21.3). v2.0 pulled forward.
- **v0.11** — 2026-06-16 — **Deepen the ripples** (Stan: "deepen the ripples"). Two more norms-pattern cross-theater channels (DESIGN §21.3–21.4). **`sim/markets.py`** — world **arms** price (rises with global conflict → lifts insurgent ExternalSupport everywhere via `arms_mult`) and **oil** price (rises with petro-state instability → drags home Domestic via the ledger). **`rules/patrons.json`** + generalized **`sim/patrons.py`** — the §8 market becomes a contest of three archetypes (mercenary/investor/proxy) with a global `patron_strength` bandwagon + `rivalry` score; **sanctions now dock a patron's global reach** (deny it one state, weaken it everywhere). Both gated (`market_feedback`/`rivalry_feedback` = 0 in single-theater → markets pinned at 50, mercenary-only pull unchanged → Sahel still 10/10; >0 in grand). Both engines (+ proto "World markets & rivals" readout); `tests/test_markets.py` (9). 86 tests / 1 xfail; calibration 10/10. Scope honest: systems deepened, grand mode still not history-calibrated; ~45 nodes/ideology contagion/map-UI backlog (DESIGN §21.5).
- **v0.12** — 2026-06-16 — **The world widens** (Stan: "as many nations on earth"). Grand mode ~20 → **~40 nations across 15 theaters** (DESIGN §21.1): +20 capitals each grounded in a real 2001–2026 conflict (Sahel/Lake Chad, Horn & Great Lakes, MENA/Levant, South & SE Asia, Central Asia, the Americas/Caribbean), +40 inter-theater edges, regional patron leanings exercising the v0.11 contest (Wagner/Iran/China), petro-states feeding the oil market. Realism pass keeps resilient states (India/Egypt/Indonesia/Mexico…) standing while fragile ones (Haiti/CAR/South Sudan/Myanmar) fall. Both engines (40 mapped); `test_grand`/smoke assert the scale; determinism holds, **Sahel calibration still 10/10** (gated). `test_the_patron_contest_is_real` rewritten across seeds. 86 tests / 1 xfail. Scope honest: scale done — the expansion makes **grand-mode scoring + a grand-scaled competent policy** the next milestone (a passive 40-node world burns ~30/40; the Sahel-tuned CompetentPolicy can't yet out-score passive — not faked) (DESIGN §21.5).
- **v0.13** — 2026-06-16 — **Grand scoring scale-invariant + the winnability problem measured honestly** (DESIGN §21.6). Fixed the scale bug (absolute junta count drove `order_mult`→≈0.08 → all grand scores ≈0). New **gated** `grand_scoring` (0 single-theatre → byte-identical, calibration 10/10; 1 grand) → scale-invariant **containment** score: population-weighted quality + free fraction of capitals, dragged by consolidated blocs; kinetic ranks worst at every scale. Both engines. **Honest finding (measured, not faked):** no policy robustly beats passive at 40 nodes (benchmark containment policy wins 0–1/6 seeds across weight×bandwidth grids; more mandate bends the world only ~25→~19 juntas, an edge costs erase) — the player's levers are too *local* to bend a *global* trajectory. Ships the foundation + names the real milestone (world-scale levers: coalitions, regional commands, sanctions with teeth) rather than tuning a fake win. 87 tests / 1 xfail.
- **v0.14** — 2026-06-16 — **Regional Commands: the first world-scale lever; grand mode becomes winnable** (DESIGN §21.7). The lever the research pointed to (AFRICOM/Barkhane/MNJTF; HoI4 garrison templates): `sim/commands.py` + new `establish_command` initiative / `command` op — a standing posture over a whole **theatre** that passively **contains** insurgency across all its nodes each turn (breadth, not depth) at an **accelerating home-front cost** (treasury upkeep + triangular Domestic strain = Kennedy overstretch + Merom + Mueller), a hard cap (triage), withdrawal on politics not defeat (Barkhane). Gated by `commands_enabled` (0 single-theatre → dormant + menu-filtered → calibration still 10/10; 1 grand). New `GrandCompetentPolicy` (lead with commands, then triage, don't over-extend) **beats passive 7/8 seeds and is the best strategy, kinetic worst** — causal (ON vs OFF) and modest (~25→~21 juntas). Both engines (+ proto readout); briefing carries `theater` + own `commands`. `tests/test_commands.py` (7). Grounded in 3 parallel research passes. 95 tests / 1 xfail; calibration 10/10.
- **v0.15** — 2026-06-16 — **Coalition burden-sharing: the second world-scale lever** (DESIGN §21.8). `sim/coalition.py` + new `rally_coalition` initiative / `coalition` op — one cohesion gauge whose `burden_share` (= `coalition_max_share·cohesion/100`) lightens both the treasury upkeep and the accelerating home strain of Regional Commands. The thesis again (Olson–Zeckhauser free-riding): cohesion bleeds each turn, frays faster under an ascendant rival (§8 rivalry) and the more commands you lean on it, must be re-rallied (with backfire) — settles mid-range, never full (NATO 2% reality). Gated by `coalition_enabled` (0 single-theatre → dormant + menu-filtered → calibration 10/10; 1 grand). Coalition lifts `GrandCompetentPolicy` 6/8 → **7/8 vs passive**, ~19 → ~22 mean, ~21 → ~18 juntas — earned (costs capital, decays) and bounded (world mostly burns); pures lose, kinetic worst. Both engines (+ proto Rally action + cohesion readout); briefing carries own `coalition`. `tests/test_coalition.py` (7). Grounded in a coalition/alliance-mechanics research pass. 102 tests / 1 xfail; calibration 10/10.
- **v0.16** — 2026-06-16 — **The Emergency Powers track; the thesis-as-test-suite is COMPLETE** (DESIGN §7; the last xfail promoted). Full escalating ladder, real-measure-grounded, built from existing ops: I Surveillance (existing) → II **Administrative Detention** (`administrative_detention`: attrition + control, Belmarsh/MISA/Micas) → III **Censorship & Martial Law** (`martial_law`: big governance/Order boost, India 1975/Turkey). Each cheap, front-loaded, *genuinely powerful* (tempting) + permanent one-way **Drift**. New `drift_score_cost`: Drift is now a **direct** score cost as well as the IntegrityMultiplier (blood and backsliding on one ledger; the loss is mechanical, not a Frostpunk-style scold). `test_emergency_powers_tempting_but_scored` **ENFORCED**: EmergencyPowersPolicy now has stronger raw (~16 vs ~10) yet worse final (~−9 vs ~−1) than kinetic — win ugly, score poorly. **All four thesis tests now true by force.** Both engines (drift cost mirrored in proto). Calibration-safe (passive/competent/kinetic never drift → 10/10, other thesis tests untouched). Grounded in a states-of-emergency research pass. 103 tests / 0 xfail.
- **v0.17** — 2026-06-16 — **Show, don't tell — the map starts speaking** (UI pass 1; per Stan: less wordy, understand by inference from the screen, each idea a concrete object; DESIGN §13.3 legibility principle extended). The board carries state: insurgent strength = a **red ring** (thickness = strength), node colour = who-rules, your Regional Command = a **flag**, petro-state = **oil drop**, capital = **★**; a dense 40-node board drops labels → **tap to identify** (interactive). Resources are objects in the header: 🏛️ Mandate 💰 Funds 🏠 Home 🤝 Allies 🛡️ Local ⛓️ Drift (words → tooltips + a visual Key legend). New `proto/preview.mjs` renders the board headlessly (SVG→PNG via cairosvg) — caught a real bug (node radius exploded at grand scale; now log-scaled+capped). Proto/UI only → engine/rules/calibration untouched (103 tests, smoke, 10/10 calibration all hold). Honest scope: 40-node board legible but dense; pan/zoom next.
- **v0.18** — 2026-06-16 — **A situation map, not a void** (UI pass 2; Stan: "can we make it prettier? realistic backgrounds?"). Backdrop: deep-ocean radial gradient + cartographic graticule + a soft **landmass hull under each theatre's nodes** (adjacent theatres merge into coastline) + faint region labels. The 40-nation world **re-laid-out** with breathing room (rough geography, grouped by theatre) so nodes stop overlapping and land shows; node/heat sizes tuned down. `preview.mjs` gained the backdrop and drove iteration. Honest split: vector situation-map aesthetic = now (greybox); photoreal terrain + 3D situation-room + motion = production client (Godot §13.4). Proto/UI only → 103 tests/smoke/calibration 10/10 hold. Next: pan/zoom for the dense board.
- **v0.19** — 2026-06-16 — **Pan & zoom — the dense board becomes legible up close** (UI pass 3). The SVG `viewBox` is now drag-to-pan, pinch/wheel/button to zoom (pointer events + `getScreenCTM` cursor-anchored zoom, tap-vs-drag so panning never mis-selects). Zoomed out = colour + heat + tap-to-identify; **zoom in and the names/stars return** (re-render across `view.w < 46`). A `+/−/⤢` control on the map. Pure proto interaction → engine/rules/calibration untouched (103 tests, smoke, 10/10 hold). `preview.mjs` got a viewBox arg for zoomed renders.
- **v0.20** — 2026-06-17 — **Motion — the map reacts to a turn** (UI pass 4; Stan: "animate changes — a junta falling, a command planting, heat flaring"). The board snapshots itself before a turn resolves; after the re-render, the nodes that changed get a one-shot animation — a fallen capital flashes, a flared theatre pulses its heat ring, a new command unfurls its flag. Driven by a pre/post diff (gov rank worsened · est. strength jump >8 · new theatre command) adding transient CSS-keyframe classes to just those node groups, auto-removed before later re-renders. Honours `prefers-reduced-motion`. Pure proto → 103 tests/smoke/calibration 10/10 hold. Rounds out the interactive UI arc (objects → situation-map → pan/zoom → motion).
- **v0.21** — 2026-06-17 — **The panels speak in objects too** (UI pass 5; finishing "less wordy / ideas as objects"). Action list + region detail now match the header/map: costs read 🏛️ n · 💰 n; action family an icon (⚔️/🏗️/🕵️/🤝/🏛️), risk a ⚠️ % (channel → tooltip), target a 📍; effect previews lead with 🏠/🤝/🛡️/🔥/⛓️ icons; region stats iconified (💥 Forces · 🏚️ Dug-in · ⚡ Activity · 🔦 Exposure · 🏴 Foreign patron…). Doctrinal terms stay in tooltips (§13.3 gloss). Proto/UI only → 103 tests/smoke/calibration 10/10 hold. Completes the interactive UI arc.
- **v0.22** — 2026-06-17 — **The Closed Society layer ships as content** (Stan's founding ask: the harsh realities of authoritarian rule; DESIGN §20.2). Seven `any_junta`-gated, sober, sourced event cards in both the base deck (base+grand) and the Sahel-arc deck: The Cordon (Moura ≥500), What Everyone Knows (African-Initiative FIMI), Until Further Notice (party dissolution), Mobilised (punitive conscription), The Quiet (media-ban ladder), Who Is Left to Count (UN panel vetoed), Struck Off (2026 sanctions reversal). Each = a choice to **bear witness (fund the §20.4 Exposure track) or look away** (costs Local + feeds grievance). Grounded in a verified research pass; added to `rules/sources.json` with URLs + confidence flags (§20.6); `exposure` added to the event-effect schema enum (no new ops). Calibration-safe (fire only post-junta, don't move junta-timing) → 10/10; 103 tests / 0 xfail. (A coalition inner/outer tiering was prototyped first and reverted — measured ~0 effect; commands don't persist long enough for it to matter.)
- **v0.23** — 2026-06-18 — **Named people, kept** (§20.2/§19.10): the closed-society realities made personal. Four composite people met then lost — Aminata (teacher→new curriculum), Ousmane (reporter→mobilised), Dr. Fatou (clinic→requisitioned), Judge Ibrahim (court→adjourned) — each a two-card *met*→*kept* arc in the Sahel deck, the *kept* gated by a new general **`requires_fired`** chain predicate (both engines + schema). Proto end-screen post-mortem closes with **"And the people you met"** (fate red if their region fell, green if held). *kept* losses on choice 1 (passive never picks) → calibration 10/10; chain-integrity test added; 103 tests / 0 xfail. People are composites; fates documented (sources.json).
- **v0.24** — 2026-06-26 — **Balance audit — the horizon-dominance finding, surfaced not hidden** (DESIGN §19.7). New player-side instrument **`harness/audit.py`** (the companion to `calibrate.py`: calibrate grades the passive *world* against history, audit grades the *player* — score spread, ending mix, the §19.7 no-dominant-strategy gate) re-runs every archetype at **both** the 120-turn subset the enforced thesis test checks and the arc's full **168-turn horizon**. Finding: **§19.7 holds @120 but breaks @168** — pure hearts-minds overtakes balanced play once the run is long enough. Named, not papered over: a 5th thesis assertion `test_no_pure_strategy_dominates_at_the_full_horizon` asserts the destination and **xfails** until closed (the xfail test is the gate; the audit harness is report-only — never fails CI). (The mechanism was first mis-attributed here; **v0.25 diagnosed it by measurement and corrected the story** — recording the wrong first guess is the discipline.) Docs + tests + harness only; no rules/sim/proto/calibration change → calibration 10/10; **104 tests / 1 xfail** (the named destination).
- **v0.25** — 2026-06-26 — **The horizon finding, diagnosed — and the benchmark sharpened** (DESIGN §19.7). The measured follow-through on v0.24: before building the named fix I measured *why* balanced play loses at 168t, and **the v0.24 hypothesis was wrong** (corrected everywhere, per *measure-never-assume*). Real mechanism: capital collapse is driven by insurgent **strength ÷ capital governance** (`factions.collapse_rolls`), **not grievance directly**, so keeping juntas low means keeping total **strength** low — and all-in development aimed at the worst-**grievance** regions starves recruitment at the source and does exactly that; over a long run that concentration wins because **development has no diminishing-returns / co-option downside yet** (Galula under-modelled). *Not* "conceded countryside drops out of the average" (Stabilization means over all nodes, rewards holding ground — balanced's is higher) and *not* security costs piling up (benchmark barely uses force). **Shipped, calibration-safe:** `CompetentPolicy` develops where **grievance** is worst (the prevention-at-source lever it always named), not where forces are — §19.7 120t margin ~1→**~9** (competent 12→20, dominates every pure), 168t gap 1.3→**4.8**; policies never touch the passive world → **10/10 by construction**. **Deeper fix, direction corrected by a 2nd measurement:** grip-scaled development **co-option** (obvious candidate) was built + measured and pointed the **wrong way** (at 168t dropped balanced play more than hearts-minds — co-option scales with insurgent control, and the lowest-control strategy is all-in prevention); **reverted**. Real requirement: a **grievance-independent insurgent sustainment channel** (coercion / external arms) — a calibration-sensitive world-dynamics milestone; `test_no_pure_strategy_dominates_at_the_full_horizon` still **xfails** (4.8 < 8.3). Sim + docs only; calibration 10/10; **104 tests / 1 xfail**.
- **v0.26** — 2026-06-28 — **Plainer words, and a coach** (UI pass 6; Stan: "less jargonistic… the learning curve is too hard, and not just for skill, but linguistic understanding"; answers chosen: *replace jargon outright* + *coach me as I play*). Specialist vocabulary removed from the player UI in favour of everyday words — Mandate→**Backing**, Funds→**Money**, Local legitimacy→**Trust**, Grievance→**Anger**, Govt control→**Control**, Entrenchment/Dug-in→**Dug in**, Activity→**Violence**, Visibility→**Your view**, Exposure→**Spotlight**, Foreign patron→**Foreign backer**, Drift→**Overreach** — plus action names (HUMINT Network→**Informant network**, Development Program→**Aid & services**, Seek UN Mandate→**Seek UN approval**, …) and the end-screen score table. Teaching is now *by doing*: a skippable five-step first-run **coach** walks the loop one beat at a time (find the red rings → tap a region → pick an action → end the month), replacing the up-front wall; the Key-tab glossary trimmed to a short scannable explainer. Doctrinal terms kept in the rules data + Sources appendix (grounding preserved, never required to play). Fixed a latent `renderHeader` crash (missing `#hSource` element silently broke the standing bars). Proto/UI only — sim/rules/calibration untouched; smoke + snapshot green; verified in a headless browser (Playwright/Chromium). 104 tests / 1 xfail unchanged.
- **v0.27** — 2026-06-28 — **Sources page made readable, scholarship intact** (Stan picked "dejargon the sources" *but* warned: *words have weight — don't lose scope to synonyms*). Judgment: the cited research's terms (legitimacy, counterinsurgency, attrition, FIMI, transnational repression…) are load-bearing and link to the literature — swapping them loses scope — so they're **kept verbatim**; instead the Sources section gets a one-line **translation key** (Backing = mandate · Trust = local legitimacy · Anger = grievance · Spotlight = exposure · Overreach = drift · …) so it reads in the new vocabulary without softening a single term. DESIGN §13.3 now records the canonical **plain ↔ model** map and the standing rule: model/doctrinal terms are the development language (code/tests/docs), the UI is a plain skin over them (supersedes the v0.5 gloss-in-tooltip rule for the UI). Proto + docs only; no engine/rules/calibration change; 104 tests / 1 xfail unchanged.
- **v0.28** — 2026-06-28 — **Thesis suite COMPLETE — the 168t dominance gap closed** (DESIGN §19.7). The fifth thesis assertion (`test_no_pure_strategy_dominates_at_the_full_horizon`), xfailing since v0.24, is now **enforced** — all five true by force. Journey (measure, never assume): v0.24 found balanced loses @168t & guessed wrong; v0.25 measured the truth (collapse = strength÷governance, not grievance → long-game winner ends with the *smallest* insurgency); two world-dynamics fixes (grip co-option; low-grievance coercion) were built + **measured wrong-way** (both reward the lowest-strength strategy = pure prevention) + reverted. The fix is a **calibration-safe benchmark fix**: `CompetentPolicy` → **concentrated political-primacy** (prevent at scale via grievance-dev **and** shrink the force via negotiation; no budget wasted on low-leverage support tools). Robustly tops every pure @120 **and** @168 (≈21.7/10.9 vs best pure ≈11.0/8.3, across 5/8/20-seed batteries — not seed-overfit). Only the player benchmark changed → passive WORLD untouched → **calibration 10/10; 105 tests / 0 xfail.** Honest caveat (documented): the win is from *concentration*, not *breadth* — support tools (intel/umbrella/exposure) too low-leverage to earn budget at the horizon, a separate tuning question. Sim + harness + docs only.
- **v0.29** — 2026-06-28 — **"The world notices" — the founding vision made content** (DESIGN §20.4; Stan's founding ask: *fund think-tanks/academics so the reality reaches the global stage and pressure rises*). New **`min_exposure`** event predicate (both engines + schema) gates cards on the documentation the player has funded; passive never funds research → Exposure stays 0 → cards **never fire unprompted (calibration-safe by construction)**. Three grounded, sober cards (base + Sahel-arc decks), each a real trade: **The Report Lands** (funded research published & cited — Global Witness/Sentry→Magnitsky pipeline), **They Reach the Researchers** (transnational repression — the human cost, named), **Cited at the Council** (reaches the UNSC; regime cries "neocolonial fabrication" — EEAS–VIGINUM FIMI; chained to The Report Lands via `requires_fired`). Also gives the otherwise-low-leverage exposure tool narrative purpose (a *content* answer to the v0.28 breadth caveat, not a risky balance change). Verified: all 3 fire for a funding player & the chain holds; nothing in the passive/pure/competent test policies funds research → **calibration 10/10; 105 tests / 0 xfail.** Both engines + snapshot; grounded in the existing record (Freedom House; EEAS–VIGINUM; The Sentry/Global Witness; OCCRP). Bundles a §19.7 docs softening (breadth isn't punished in normal play; only the extreme grind favours focus — a deep budget property, not a defect).
- **v0.30** — 2026-06-28 — **Production direction: thin-client contract + Godot scaffold** (DESIGN §13.4; Stan picked "production"). The verifiable core: **`docs/CLIENT.md`** specifies how any client consumes the JSON rules core — embed-an-engine vs display-one, rules loading + §18.9 scenario layering, the turn-loop/engine contract, the render-state shape, score/ending contract, and binding display rules (plain words + the canonical plain↔model map; show-don't-tell) — grounded in the tested Python sim + JS proto. **`client/godot/`** is a Godot 4 *starting skeleton* (loads rules with scenario layering, renders the Sahel arc's starting state read-only in plain words). **Explicitly UNVERIFIED** — no Godot binary in the build env, so not run; **isolated** under `client/godot/` (touches nothing in sim/proto → cannot affect the tested game). Honest by discipline: the contract documents what's true; the scaffold is quarantined + flagged, not pretended to work. No sim/rules/proto/test change → **calibration 10/10; 105 tests / 0 xfail** unchanged. Godot was checked for (`which godot` → absent), hence the can't-run-here honesty.
- **v0.31** — 2026-06-28 — **Playtest fix: the end-screen `sorted is not defined` crash.** Found by the first true end-to-end browser playthrough (drove a full 60-turn game; earlier QA only rendered turn-1 states, so the end-screen path had never run). The proto's end-screen deceptive-calm reveal called `sorted()` — a helper **private to the engine IIFE**, invisible to the separate UI-layer IIFE — so it threw when a finished game drew its post-mortem (a Python-ism left across the JS scope boundary). Fixed: the UI layer gets its own `sorted` (`Object.keys(x).sort()`). Verified by playthrough — full game completes; end screen renders the plain labels (Peace held · Order · Staying clean · Costs · Final) + post-mortem; events fire and are choosable in-browser (probe: 17 modals / 40 turns, no page errors). Proto/UI only; engine/rules/calibration untouched; **105 tests / 0 xfail; calibration 10/10.** (Lesson: render-only QA misses turn-loop/end-screen bugs — playtest to the horizon.)
