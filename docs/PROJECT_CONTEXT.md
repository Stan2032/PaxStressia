...
- **v0.19** — 2026-06-16 — **Pan & zoom — the dense board becomes legible up close** (UI pass 3). The SVG `viewBox` is now drag-to-pan, pinch/wheel/button to zoom (pointer events + `getScreenCTM` cursor-anchored zoom, tap-vs-drag so panning never mis-selects). Zoomed out = colour + heat + tap-to-identify; **zoom in and the names/stars return** (re-render across `view.w < 46`). A `+/−/⤢` control on the map. Pure proto interaction → engine/rules/calibration untouched (103 tests, smoke, 10/10 hold). `preview.mjs` got a viewBox arg for zoomed renders. Closes the "interactive & intuitive" UI arc; production pan/zoom + motion inherit to the Godot client.
